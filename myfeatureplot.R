myfeatureplot=function(data,filename,genename,pt.size=0.5,min.cutoff="q5",reduction="tsne",max.cutoff="q99",cols=c("lightblue1","lightgreen","darkorange","firebrick1")){
  ...
  while(featureplot_no>1){
    ...
    print(g)
    dev.off()
    png(paste0("Featureplot_",filename,"_",as.character(featureplot_count),".png"),width = 1800, height = 980)
    g=FeaturePlot(object = data,reduction = reduction,features = genename[(6*(featureplot_count-1)+1):(6*featureplot_count)],cols = cols, pt.size = pt.size,ncol=3,min.cutoff = min.cutoff,max.cutoff = max.cutoff)
    print(g)
    dev.off()
  ...
  if(featureplot_no>0){
    pdf(paste0("Featureplot_",filename,"_",as.character(featureplot_count),".pdf"),width = 6.7*ifelse((marker_no-(6*(featureplot_count-1)))>=3,3,(marker_no-(6*(featureplot_count-1)))), height = 5.1*ceiling((marker_no-(6*(featureplot_count-1)))/3))
    g=FeaturePlot(object = data,reduction = reduction, features = genename[(6*(featureplot_count-1)+1):marker_no],cols = cols, min.cutoff = min.cutoff,
                  max.cutoff = max.cutoff,pt.size = pt.size,ncol=ifelse((marker_no-(6*(featureplot_count-1)))>=3,3,(marker_no-(6*(featureplot_count-1)))))
    print(g)
    dev.off()
    png(paste0("Featureplot_",filename,"_",as.character(featureplot_count),".png"),width = 600*ifelse((marker_no-(6*(featureplot_count-1)))>=3,3,(marker_no-(6*(featureplot_count-1)))), height = 490*ceiling((marker_no-(6*(featureplot_count-1)))/3))
